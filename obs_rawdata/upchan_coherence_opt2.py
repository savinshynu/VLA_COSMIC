...
def main(
    ...
    dh = open(os.path.join(savefig_directory, f"delays_{plot_id}.csv"), "w")
    dh.write(",".join(
            [
                "Baseline",
                "total_pol0",
...
def calculate_geo_delay(metafile, mjd_time):
    ...
    # screwed up dictionary again
    ants_mcast = data['META_ANT']['AntennaProperties']
    ants_dict = {}
    for i in range(len(ants_mcast)):
        ants_par = ants_mcast[i]
        ants_dict[ants_par['name']] = {'X' : ants_par['X']['value'], 'Y' : ants_par['Y']['value'], 'Z' : ants_par['Z']['value']}
