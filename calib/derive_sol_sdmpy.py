...
#Picking the first baseline
data_bls0_rr = data_avg[0,:,0,:,0].flatten()
data_bls0_ll = data_avg[0,:,0,:,1].flatten()

plt.subplot(1,2,1)
#plt.plot(freqs, 10*np.log10(np.abs(data_bls0_rr)), '.', label = "RR")
#plt.plot(freqs, 10*np.log10(np.abs(data_bls0_ll)), '.',  label = "LL")

plt.plot(freqs, np.abs(data_bls0_rr), '.', label = "RR")
#plt.plot(freqs, np.abs(data_bls0_ll), '.',  label = "LL")
...
plt.subplot(1,2,2)
plt.plot(freqs, np.angle(data_bls0_rr, deg = True), '.', label = "RR")
#plt.plot(freqs, np.angle(data_bls0_ll, deg = True), '.',  label = "LL")
plt.ylabel("Phase (deg)")
plt.xlabel("Frequency (GHz)")
plt.suptitle(f"Before calibration: {bls[0]}")
plt.legend()
plt.show()
...
#Picking the first baseline
cal_data_bls0_rr = cal_data_avg[0,:,0,:,0].flatten()
cal_data_bls0_ll = data_avg[0,:,0,:,1].flatten()

plt.subplot(1,2,1)
#plt.plot(freqs, 10*np.log10(np.abs(cal_data_bls0_rr)), '.', label = "RR")
#plt.plot(freqs, 10*np.log10(np.abs(cal_data_bls0_ll)), '.',  label = "LL")

plt.plot(freqs, np.abs(cal_data_bls0_rr), '.', label = "RR")
#plt.plot(freqs, np.abs(cal_data_bls0_ll), '.',  label = "LL")
...
plt.subplot(1,2,2)
plt.plot(freqs, np.angle(cal_data_bls0_rr, deg = True), '.', label = "RR")
#plt.plot(freqs, np.angle(cal_data_bls0_ll, deg = True), '.',  label = "LL")
plt.ylabel("Phase (deg)")
plt.xlabel("Frequency (GHz)")
plt.suptitle(f"After calibration: {bls[0]}")
plt.legend()
plt.show()
