...
def main(args):
    ...
    if  args.time_delay:
        ...
        ax0.set_title(f"{ants[0]}-{ants[1]}: time delay = {tlags[tmax_pol0]} ns")
        ax0.legend()
        ...
        ax1.set_title(f"{ants[0]}-{ants[1]}: time delay = {tlags[tmax_pol1]} ns")
        ax1.legend()
        ...
        if args.plot:
            ...
            plt.savefig(f"time_delay_{os.path.basename(args.dat_file)}.png", dpi = 150)
            plt.close()
