...
from turbo_seti.find_doppler.data_handler import DATAHandle, DATAH5
from blimpy.io.hdf_reader import H5Reader
from utils import comp_bp_coarse
...
"""
hf = h5py.File(h5file, mode='r')
print (hf.keys())

print (f"shape from h5py: {hf['data'].shape}")

fb = Waterfall(h5file)

print (f"shape from blimpy waterfall: {fb.data.shape}")
print (f"num of tints from blimpy: {fb.n_ints_in_file}")


doppler = FindDoppler(h5file,
                      max_drift = 4, # Max drift rate = 4 Hz/second
                      snr = 10,      # Minimum signal to noise ratio = 10:1
                       # Blimpy calculate this for earlier BL data, make sure each file has correct coarse channels
                       # This is where the turboSETI output files will be stored.
                     )
       
#for cchan_obj in doppler.data_handle.cchan_list:
#    print (cchan_obj)
#print (doppler.n_coarse_chan)
#print (doppler.kernels)

cchan_dict = doppler.data_handle.cchan_list[0]
datah5_obj = DATAH5(cchan_dict['filename'],
                  ...
                  gpu_backend=False,
                  precision=doppler.kernels.precision)
...
print(f"tsteps = {tsteps}, tsteps_valid = {tsteps_valid}, tdwidth = {tdwidth}, fftlen = {fftlen}, nframes = {nframes}")



hob = H5Reader(h5file)
print (hob.n_ints_in_file)
"""
...
from turbo_seti.find_doppler.kernels import Kernels, Scheduler
from turbo_seti.find_doppler.data_handler import DATAHandle, DATAH5
from turbo_seti.find_doppler.file_writers import FileWriter, LogWriter
from turbo_seti.find_doppler.helper_functions import chan_freq, comp_stats
...
def msearch_coarse_channel(cchan_dict, fd, dataloader=None, logwriter=None, filewriter=None):
    r"""
    ...
    nframes = tsteps_valid
    shoulder_size = datah5_obj.shoulder_size
    datah5_obj.drift_rate_resolution  = (1e6 * np.abs(datah5_obj.header['DELTAF'])) / datah5_obj.header['obs_length']   # in Hz/sec
    #print(f"obs length: {datah5_obj.header['obs_length']}, spectra shape: {spectra.shape}")    
    #print('flagging={}, tsteps={}, tsteps_valid={}, tdwidth={}, fftlen={}, nframes={}, shoulder_size={}'
    ...
    #print(f"tree_findoppler:{tree_findoppler.shape}")
    ...
    #print(f"tree_findoppler_flip:{tree_findoppler_flip.shape}")
    ...
    #print(f"Drift rate nblock : {drift_rate_nblock}")
    # Transfer data to device
    spectra = fd.kernels.xp.asarray(spectra, dtype=fd.kernels.float_type)
    
    #fbp = Waterfall(datah5_obj.filename)
    #bp = comp_bp_coarse(spectra[:int(fbp.n_ints_in_file),:])
    #spectra /= bp


    #plt.imsave('spec.png',spectra)
    #plt.pcolormesh(spectra)
    #plt.plot(range(spectra.shape[1]),np.mean(spectra[:18,:],axis =0))
    #plt.show()
    ...
    #print (f"Drift low = {drift_low}, Drift high = {drift_high}, Range of drift block: {len(range(drift_low, drift_high))}")
    for drift_block in range(drift_low, drift_high):
        ...
        if drift_block <= 0:
            ...
            #print(f"Drift range: {complete_drift_range.shape}")
            bool_selected = complete_drift_range >= -fd.max_drift
            #print(f"bool_selected shape: {bool_selected.shape}")
            ...
            for k, drift_rate in enumerate(complete_drift_range[bool_selected]):
                ...
                #if drift_block == drift_low:
                #    print(f"drift rate: {drift_rate}, Index: {indx}")

                #if k == 1:
                #    print(f"Spectrum shape: {spectrum.shape}, index val:{indx}, tdwidth:{tdwidth}")
        ...
        * n_partitions governs the maximum number of partitions to run in parallel.
        ...
        if n_partitions == 1:
            ...
            for cchan_obj in self.data_handle.cchan_list:
                msearch_coarse_channel(cchan_obj, self, dataloader=sched, filewriter=filewriter, logwriter=logwriter)
        # Run Parallel version via dask
            ...
            if progress_bar == 'y':
                with ProgressBar():
                    b.map(msearch_coarse_channel, self).compute()
            else:
                b.map(msearch_coarse_channel, self).compute()
...
base_dir = '/home/cosmic/savin/seti_search/MK/MK_dat/MK_HighFreqRes/'

fil_files = glob.glob(base_dir+'*.fil')

for i,fil in enumerate(fil_files):
    newdir = base_dir+'file%d/' % (i)
    os.mkdir(newdir)
    os.chdir(newdir)
    os.system("fil2h5 " + fil)

    
    h5file = newdir+os.path.splitext(os.path.basename(fil))[0]+'.h5'
    print (newdir,h5file)
    ...
    dop1 = mFindDoppler(h5file,
                      max_drift = 4, # Max drift rate = 4 Hz/second
                      snr = 10,      # Minimum signal to noise ratio = 10:1
                      #coarse_chans=[0],          
                      n_coarse_chan = 512, # Blimpy calculate this for earlier BL data, make sure each file has correct coarse channels
    ...
    h5_list = sorted(glob.glob(os.path.join(data_dir, h5file)))
